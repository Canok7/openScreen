...
void cmdTaskFunc(void *live555) {
    ...
    auto *obj = (Clientlive555 *) live555;
    switch (obj->getCmd()) {
        case Clientlive555::PAUSE_SUBSESSION:
            break;
        case Clientlive555::PLAY_SUBSESSION:
            break;
        case Clientlive555::TEARDOWN_SUBSESSION: {
            ALOGD("control [%s%d]", __FUNCTION__, __LINE__);
            ...
            break;
        case Clientlive555::CMD_UNKNOWN:
            break;
        default:
...
void
Clientlive555::start(char *url, IRtspClientNotifyer *notifyer, bool bTcp, unsigned udpReorderTimeUs,
                     unsigned cachTimeUs) {
    mNotifyer = notifyer;
...
const char *Clientlive555::getUrl() const {
    return mUrl;
}

ourRTSPClient *Clientlive555::getOurRtspClient() {
    return mRtspClient;
}

char *Clientlive555::getEventLoopWatchVariable() {
    return &eventLoopWatchVariable;
}

void Clientlive555::setCmdTrigId(EventTriggerId id) {
    mCmdEventId = id;
}
...
void Clientlive555::openURL(UsageEnvironment &env, char const *progName, char const *rtspURL) {
...
void Clientlive555::onRtspClinetDestoryed(void *pram) {
...
void Clientlive555::onNewStreamReady(std::shared_ptr<MediaQueue> pram) {
...
void Clientlive555::control(SRC_CMD cmd, void *data) {
    mCmd = cmd;
    mCmdData = data;
