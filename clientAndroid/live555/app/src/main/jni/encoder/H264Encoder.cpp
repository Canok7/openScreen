...
//        AMediaFormat_setInt32(format, "color-format", OMX_COLOR_FormatAndroidOpaque);
    ...
    media_status_t status = AMediaCodec_configure(mAMediaCodec,format,nullptr,nullptr,AMEDIACODEC_CONFIGURE_FLAG_ENCODE);//解码，flags 给0，编码给AMEDIACODEC_CONFIGURE_FLAG_ENCODE
    if(AMEDIA_ERROR_BASE == status){
        AMediaFormat_setInt32(format,"prepend-sps-pps-to-idr-frames",0);
        status = AMediaCodec_configure(mAMediaCodec,format,nullptr,nullptr,AMEDIACODEC_CONFIGURE_FLAG_ENCODE);//解码，flags 给0，编码给AMEDIACODEC_CONFIGURE_FLAG_ENCODE
    }
